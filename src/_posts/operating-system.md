...
-  基本的地址变换机构
   ...
   首先，获取页号和偏移量，然后根据页表寄存器中的页面始址与页号获得该页的实际物理地址（开始位置），然后再加上页内的偏移量就得到了要找的物理地址
...
-  循环扫描 CSCAN
   ...
   SCAN 对两端的磁道不公平，为此规定磁头只能单向移动，一旦该方向无新的请求，则直接回到起点一端，继续沿该方向扫描

-  N-STEP-SCAN

   将请求分为 N 个长度为 M 的队列，队间用 FCFS 处理，队内用 SCAN 处理
...
-  FSCAN
   ...
   就是上面的 N=2 的特例，将磁盘请求队列分成两个子队列，一个是由当前所有请求磁盘 I/O 的进程形成的队列，由磁盘调度按 SCAN 算法进行处理。在扫描期间，将新出现的所有请求磁盘 I/O 的进程，放入另一个等待处理的请求队列。这样，所有的新请求都将被推迟到下一次扫描时处理
