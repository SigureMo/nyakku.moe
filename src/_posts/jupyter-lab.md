...
什么工具更适合在云端炼丹呢？基于在本地使用 VSCode 的流畅体验，我最初尝试的是 VSCode 的 Remote-SSH 扩展，经过一段时间的体验之后，发现 VSCode 的远程连接很不稳定（也可能是 VPN 不稳定的原因），而且目前内置 Jupyter 还只是起步阶段，使用体验并不是很好。而直接使用 Jupyter Notebook 的话，多个文件以及终端之间的切换与管理又非常不方便。JupyterLab 刚好结合了两者的优点，将多 Tab 标签页、终端以及 Jupyter 集成起来，最重要的是它还支持丰富的扩展~
