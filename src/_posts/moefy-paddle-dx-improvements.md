...
如果一个工具需要专门分析某种语言一些格式上的问题的话，在词法上分析 Token Stream 是比较合适的，不过这样的需求一般也不会太多，目前我了解到的也只有 pycodestyle 这一个工具使用的是这种方式。大多数工具都是在进一步处理成的 AST 上进行操作的。

### 再进一步直接在 AST 上操作

经过语法分析后，我们就可以得到 AST 啦。AST 是抽象语法树（Abstract Syntax Tree）的缩写，所以很明显啦，它是一个树形结构。这里将之前我在进行[单测报错信息优化](https://github.com/PaddlePaddle/community/blob/master/rfcs/CodeStyle/20220805_code_style_improvement_for_unittest.md)时的一个简单的例子来说明一下 AST 的结构。
...
不过非常巧的是 [HerringtonDarkholme](https://github.com/HerringtonDarkholme) 就在前段时间开源了一个基于 AST 的文本搜索替换工具 [ast-grep](https://github.com/ast-grep/ast-grep)，整体想法和我的想法基本上一致，当我看到这个工具的名字的时候其实就确定了。
...
嘿嘿，也许真的会有些需求是需要我们自己动手写各种各样的工具的，比如我们有一些定制化的重构需求，或者在引入新的新的工具的时候。
