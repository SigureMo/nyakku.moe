...
有时候 Git 不小心提交了一些二进制文件，当随着 repo 越来越庞大，这些二进制文件就会像滚雪球一般越来越大，而且即便删除掉这些二进制文件，仍然会存在于历史之中，为了解决该问题，就需要一些针对二进制文件的处理手段啦～
...
Git LFS 是 github 推出的大文件存储服务，它是让 git 只保存二进制文件的 hash，而二进制文件将会存在 Git LFS 服务器中，当然，这可以有效防止 Git 历史的增长，但……免费只有 1G 空间，而且每月还限 1G 带宽，虽然看起来挺大的，但有一个月我频繁提交博客，CI 频繁地从 LFS 获取图片，结果居然真的差点就用到了 1 个 G 的带宽……之后我就开始另寻他法……
