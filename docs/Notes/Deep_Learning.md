...
4. 我们要求$d\theta_{approx}$和$d\theta$，但我们现在只有各个偏导组成的向量，要怎么度量两个向量是否彼此接近呢？我们可以使用$d\theta_{approx}[i]-d\theta[i]$的欧几里得范数$||d\theta_{approx}-d\theta||_2$，然后对向量的长度进行归一化，也就是$\frac{||d\theta_{approx}-d\theta||_2}{||d\theta_{approx}||2+||d\theta||_2}$，Python的话，可以使用np.linalg.norm(grad)
来求范数
