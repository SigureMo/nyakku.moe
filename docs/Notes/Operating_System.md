...
![OS18](../Images/OS18.png)

emmm 虽说共用一个进程吧，但是每个页表都要记录该进程的各个页

相对来说，下面的分段式就很好看啦~

![OS19](../Images/OS19.png)
...
-  确定从何处调入页面
   ...
   外存分为 - 文件区 离散的分配方式，读写慢 - 对换区 连续的分配方式，读写快
...
-  单缓冲
   ![OS21](../Images/OS21.png)
-  双缓冲
   ![OS22](../Images/OS22.png)

#### 5.3.3 循环缓冲

![OS23](../Images/OS23.png)

#### 5.3.4 缓冲池

![OS24](../Images/OS24.png)
...
-  索引结构 文件信息存放在若干不连续物理块中，系统为每个文件建立一个专用数据结构——索引表，并将这些块的块号存放在索引表中
   ...
   -  缺点
      ...
      -  索引表本身带来了系统开销，如：内外存空间，存取时间开销等
         ...
         ![OS30](../Images/OS30.png)

-  混合索引结构（多重索引，UNIX 采用）
   ![OS31](../Images/OS31.png)

   首先假设一个磁盘块 `512 Byte` ，可存储 `128` 个 `4 Byte` 的索引号，我们将主索引表分为以下区域

   -  直接地址 使用 10 个直接地址项，如果文件小于 `10 * 512 Byte` ，就用不到后面的啦
...
-  目录项（文件控制块，FCB） 用于记录文件相关信息
   ![OS32](../Images/OS32.png)

   将真正的文件属性存储在 iNode 中，更容易实现文件共享的操作

   ![OS33](../Images/OS33.png)
