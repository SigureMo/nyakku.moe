...
```
> namespace  
类比于Python的import很好理解，首先`include <pkgname>`大概相当于`import <pkgname>`，之后就可以使用这个包内的内容啦。
...
> 哭了，这是个什么鬼嘛，怎么只改个参数表就可以用其值了，连类型都不用换的，emmmmm 暂且当作是像Python、Java等语言传对象时候传manager好了
...
用class取代struct，struct相当于public的class
...
> 使用ClassName::StaticVar引用静态变量

### 3.3 Destructor and Friend
#### 3.3.1 Destructors
> 就相当于Python里的__del__啦，C++要写成~ClassName()

#### 3.3.2 Friend
> friend的对象和函数可以直接访问该对象的private

#### 3.4 CopyConstructor
`ClassName(ClassName&);`
> 使用时仅限于对象的声明时，已经声明的对象无法再通过赋值进行copy
...
> 构造函数沿着构造函数链调用所有的基类ctor，而且基类有限，而析构恰恰相反
...
比如，public、protected、private以protected方式继承将分别变为protected、protected、private
...
Array and Object  
和前面都一样，只需要加个类型转换就行，要注意的是，Object只保存数据域的数据，不会保存方法（因为方法本来就不在Object指针那里）
...
* 运算符可以看做是函数
* 函数是前缀式的
* 不能自定义新的运算符，只能重载，比如C++就不能用Python里的**
...
> 既需要作为访问器，又需要作为修改器，所以应当返回一个reference
...
当编译器遇到@obj;时
* 若operator@ 是在obj的类中的成员，则调用obj,operator@();
* 若operator@是obj的类的friend函数，则调用operator@(obj);
...
> 函数声明的后面加上const关键字可让编译器知道该函数不会其内的值
...
* throw可以直接放到函数中哦，调用函数体的位置也可以catch住的哦
* 正是这种特性，我们不需要考虑这个异常发生后如何向上传递，我们只需要写一个throw，它保证了异常可以向上传递
* 不过，在本地处理的掉的事情就不要抛出去使用异常处理

### 7.3 Exception Classes
* emmmm 就是C++有些内建异常类，继承于exception……
