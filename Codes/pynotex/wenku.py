...
def parse(url):
    types = {
        ...
        '7': {'str': 'pdf', 'parse_func': parse_xxx},
...
def parse_doc(doc_id, *params):
    ...
    for html_url in html_urls[:len(html_urls)//2]:
        ...
        json_text = re.search(r'\(([\s\S]+)\)', result.text.encode('utf-8').decode('unicode_escape','ignore')).group(1)
        json_data = json.loads(json_text)
...
def parse_txt(doc_id, *params):
    ...
    result = spider.get(txt_api.format(doc_id=doc_id, page_num=page_num, md5=md5, rsign=rsign))
